...
import { activeStocktakeSessionForHandover, confirmItemStillHere, correctMistakenOrderHandover, fulfillOrderReservationsV2, getOrderShipmentInventoryBlockers, getOrderStockHandoverState, normalizeShipmentPossessionConfirmations, OrderStockShortageError, orderShipmentInventoryBlockerMessage, orderWorkshopPendingForShipping, reconcileIssuedBeforeCheckpoint } from './domains/order-reservations.ts'
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (orderShippingMatch && request.method === 'PATCH') {
        ...
        const input = await readJson<{ shippingStatus?: unknown; shippingDate?: unknown; stockConfirmations?: unknown; observations?: unknown }>(request);
        ...
        if (humanInventoryModelEnabled && input.observations !== undefined) {
          return json({
            ok: false,
            code: 'stock_resolution_client_refresh_required',
            message: 'Эта форма отправки устарела. Обновите страницу: подтверждение наличия больше не является пересчётом всего остатка.',
          }, { status: 409 });
        }
        const normalizedStockConfirmations = humanInventoryModelEnabled
          ? normalizeShipmentPossessionConfirmations(input.stockConfirmations)
          : [];
        if (humanInventoryModelEnabled) {
          ...
          const uncoveredShortages = shortageBlockers.filter((row) => {
            ...
            return !confirmation
              || confirmation.expectedPhysicalQuantity !== physical
              || confirmation.operationQuantity !== required;
          });
        ...
        const inventoryDelivery = humanInventoryModelEnabled
          ? await fulfillOrderReservationsV2(env.DB, id, cleanText(existing.external_id), timestamp, {
              ...
              confirmedBy: cleanText(request.headers.get('X-Access-User')) || normalizeAccessRole(request.headers.get('X-Access-Role')),
            })
          : null;
        if (inventoryDelivery?.unresolved) {
