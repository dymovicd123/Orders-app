...
export async function createReturn(
  ...
  if (amount <= 0 && selectedItems.length === 0) {
    throw new Error('Выберите возвращаемый товар или укажите сумму возврата денег.');
  }
  if (amount > 0 && !paymentMethod) {
    throw new Error('Выберите способ возврата денег. Для возврата товара без денег сумма может быть 0.');
  }
  ...
  if (!returnId) {
    ...
    const createReturnStatements: D1PreparedStatement[] = [
      db.prepare(
    ...
    if (amount > 0) {
      createReturnStatements.push(refundMoneyEventStatement(db, {
    ...
    const [insertReturn] = await db.batch(createReturnStatements);
