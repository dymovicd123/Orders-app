...
assert.ok(modal.includes('В заказе: <strong>{draft[field]}</strong>'), 'Unknown reference value no longer explains the raw value in user language')
assert.ok(modal.includes('<small>Возможно</small>'), 'Reference typo flow no longer explains its recommendation')
assert.ok(modal.includes('Другой вариант'), 'Existing reference picker is no longer offered')
assert.ok(modal.includes('setCreatingReference(field)'), 'Creating a reference value is no longer an explicit action')
assert.ok(modal.includes('<label>Название<input autoFocus'), 'New reference value can no longer be edited before creation')
