...
check(controller.includes("return `до ${deadline}`") && !controller.includes('Просрочено ·'), 'CLIENT-ZAMMLER-F: ZAMMLER invoice must show a neutral deadline without overdue blame wording')
...
console.log('CLIENT-ZAMMLER-F WORKSHOP INVOICE PASSED — operational ZAMMLER invoice is separate, order-bound, deadline-aware without overdue blame wording, and excluded from the ordinary Workshop invoice')
