...
const sql = relations.slice(sqlStart, sqlEnd).replace('${requestedOrderValues}', '(?)')
...
db.exec([
  ...
  "INSERT INTO exchanges(id, order_id, refund_return_id, status) VALUES (21, 10, 3, 'completed'), (22, 10, 4, 'cancelled');",
].join('\n'))

const rows = db.prepare(sql).all(10)
const byItem = new Map(rows.filter(row => row.order_item_id != null).map(row => [Number(row.order_item_id), Number(row.returned_quantity)]))
check(rows.every(row => Number(row.committed_exchange_count) === 1), 'Combined relation query lost committed Exchange count')
