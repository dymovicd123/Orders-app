...
check(filters.includes('Доставка: ЗАММЛЕР') && filters.includes("aria-pressed={filters.deliveryType === 'zammler'}"), 'CLIENT-ZAMMLER-E: All Orders needs a clear ZAMMLER delivery filter button')
check(table.includes("filters.deliveryType === 'zammler'") && table.includes('Доставка: ЗАММЛЕР'), 'CLIENT-ZAMMLER-E: filtered state must be visible in the same Orders table')
