...
try {
  ...
  check(modal.includes("context.product ? 'В каталоге'") && modal.includes('Сохранить и продолжить'), 'Normal path must show the understood product and one confirmation')
  check(modal.includes('>Исправить вручную</button>'), 'Manual correction must remain an explicit fallback')
  ...
  check(modal.includes('Проверьте {label.toLowerCase()}') && modal.includes('<label>Название<input autoFocus') && modal.includes('В заказе:') && modal.includes('Попросите администратора добавить его') && modal.includes('>Проверить снова</button>'), 'New reusable values need explicit user-facing creation/escalation flow')
  check(modal.includes('Цвет и {draft.category === \'child\' ? \'возраст\' : \'размер\'} не указаны') && modal.includes("draft.category === 'child' ? 'возраста' : 'размера'"), 'Missing color/size-or-age require one combined explicit decision with the correct adult/child label')
  check(modal.includes('Что означает «{remainder}»?'), 'Compound-name residue must be classified, not silently asserted')
  ...
  check(!modal.includes('Подтверждённые однозначные сопоставления запоминаются'), 'Resolver must not expose technical persistence commentary in the ordinary flow')
