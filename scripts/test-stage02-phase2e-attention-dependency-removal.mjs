...
const ordersView = fs.readFileSync('src/features/sections/OrdersTableSection.tsx', 'utf8')
...
check(ordersView.includes('void openOrderStockHandover(order)') && ordersView.includes("order.stock_handover_review_needed ? 'Уточнить выдачу' : 'Выдать готовые товары'"), 'Phase2E handover still requires opening Warehouse Attention instead of the order itself')
check(app.includes("result.code === 'catalog_review_required'") && app.includes('setOrderCatalogResolutionOrder(order)'), 'Phase2E shipping catalog clarification is not available inline from the order flow')
check(app.includes("result.code === 'stock_resolution_required'") && app.includes("result.operationType === 'shipping'"), 'Phase2E shipping shortage still depends on Warehouse Attention instead of the inline resolver')
check(app.includes("const resolverOperation = isTransfer ? 'transfer' : isWriteoff ? 'writeoff' : ''"), 'Phase2E transfer/writeoff shortage handling is not attached to the operation itself')
