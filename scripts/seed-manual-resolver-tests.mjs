...
if (stockVariant) {
  add('01-KNOWN-STOCK', '01 · Известный SKU из фактического остатка',
    'Catalog resolver не должен появиться.',
    [canonicalItem(stockVariant, stockCandidate.inventorySource)])
}
if (zeroCandidate) {
  add('02-KNOWN-NO-STOCK', '02 · Известный SKU без фактического остатка',
...
add('03-NEW-PRODUCT', '03 · Полностью новое название товара',
...
add('04-GARBAGE-PRODUCT', '04 · Известный товар с мусорным названием',
...
add('07A-GARBAGE-ATTRIBUTE', '07A · Мусорное имя известной характеристики',
...
add('11-DUPLICATE-LINES', '11 · Две полностью одинаковые проблемные строки',
  'После точного подтверждения первой полной строки вторая идентичная строка должна разрешиться автоматически.',
  [
    canonicalItem(safeVariant, 'warehouse', { productName: scopedRaw }),
    canonicalItem(safeVariant, 'warehouse', { productName: scopedRaw }),
  ])

add('12-MULTI-PROBLEM', '12 · Несколько разных проблем в одном заказе',
...
add('13-WORKSHOP-UNKNOWN', '13 · Неизвестный товар Цеха',
...
add('14-NORMALIZATION', '14 · Регистр и лишние пробелы',
...
const created = []
// Create in reverse so the orders table (newest first) visibly shows TEST 01 at the top, then 02, 03…
for (const test of [...cases].reverse()) {
  const externalId = `TEST-${test.code}-${RUN_TAG}`
  const payload = {
    requestId: `manual-resolver-seed-${test.code}-${RUN_TAG}`,
    externalId,
    ...
    orderTotal: 0,
    comment: `РУЧНОЙ ТЕСТ ${test.title}. Ожидание: ${test.expected}`,
    items: test.items,
    payments: [],
  }
...
console.log(JSON.stringify({
  ...
  garbageField,
  orders: created.sort((a, b) => a.code.localeCompare(b.code, 'en', { numeric: true })),
}, null, 2))
