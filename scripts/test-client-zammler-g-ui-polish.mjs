...
check(filters.includes('Только ЗАММЛЕР') && filters.includes("current.deliveryType === 'zammler' ? 'all' : 'zammler'"), 'CLIENT-ZAMMLER-G: ZAMMLER must be a toggle filter in All Orders')
...
check(table.includes("filters.deliveryType === 'zammler'") && table.includes('Фильтр ЗАММЛЕР'), 'CLIENT-ZAMMLER-G: same table must show its active ZAMMLER filter')
