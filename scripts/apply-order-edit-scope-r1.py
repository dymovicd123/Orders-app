...
# Working-mode editing must be blocked by destructive/physical lifecycle, not by a harmless
# active -> closed bookkeeping status. Closed + not-sent orders remain ordinary corrections.
replace_exact(
...
for path, subject in [
    ...
    old = f"isAdmin || ({subject}.order_status === 'active' && {subject}.shipping_status !== 'sent')"
    new = f"isAdmin || (!['deleted', 'archived'].includes({subject}.order_status) && {subject}.shipping_status !== 'sent')"
    replace_exact(path, old, new)

# Strengthen the existing autonomy regression so this exact active-only lock cannot return.
