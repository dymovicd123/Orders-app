...
try {
  ...
  check(renderer.includes('disabled={!historicalPeriodSelected}') && renderer.includes('Перенесённые старые записи'), 'Legacy filter is not locked behind an explicitly historical range')
  check(renderer.includes('Внесено в систему') && renderer.includes('financeRecordedAt(row.eventRecordedAt)') && renderer.includes('Заказ введён:'), 'Journal does not expose actual recorded/order timestamps')
