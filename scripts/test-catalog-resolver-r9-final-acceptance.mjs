...
assert.ok(emptyBlock.includes("if (!session.current.changed) setError(purpose === 'intake' ? 'Товар уже уточнён. Закройте окно и обновите приёмку.' : 'Список уточнений пуст. Отправка не продолжена. Закройте окно и проверьте заказ.')"), 'R9 final: unchanged empty review no longer stays fail-closed for shipping/intake')
...
const closeIndex = completion.indexOf('setOrderCatalogResolutionOrder(null)', sendIndex)
