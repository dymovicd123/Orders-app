...
check(sku.includes('catalog-sku-card') && sku.includes('aria-label="Карточка позиции"'), 'exact SKU card is missing')
...
check(catalog.includes('Исправление позиции') && catalog.includes('Сохранить исправление'), 'correction editor still looks like normal SKU editing')
check(sku.includes('Другая вещь — новая позиция.') && sku.includes('Исправить ошибку') && backend.includes('catalogVariantHasOperationalUsage'), 'historical identity protection is not explained/preserved')
...
const retireStart = sku.indexOf('const retireVariant = async')
const retireEnd = sku.indexOf('\n  return (', retireStart)
check(retireStart >= 0 && retireEnd > retireStart, 'retirement function boundary missing')
const retireSource = sku.slice(retireStart, retireEnd)
check(retireSource.indexOf("setActionMessage('Позиция выведена из активного каталога") < retireSource.indexOf('const refreshed = await loadCatalogData(true)'), 'authoritative retirement success is still downstream of its refresh')
...
check(inventory.includes("...(isAdmin ? [{ value: 'catalog' as const, label: 'Товары'") && inventory.includes("openInventoryPanel(entry.value)"), 'catalog navigation is no longer restricted to admin mode')
...
console.log('W6.4 SAFE SKU CARD PASSED — exact SKU card, create-similar/correction split, guarded soft retirement, post-write reliability, UI + server admin boundaries, mobile layout and frozen Arrival are protected')
