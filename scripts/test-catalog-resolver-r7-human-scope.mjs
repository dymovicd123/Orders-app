...
try {
  ...
  check(
    review.includes("const hasCompleteRawIdentity = Boolean(")
      && review.includes("if (!hasCompleteRawIdentity)")
      && review.includes("writeAlias: false")
      && review.includes("const targetRows = identical.length ? identical : matching")
      && review.includes("writeAlias: true"),
    'Existing-variant human choice must stay single-line for incomplete identity, but may safely teach/fan out an exact complete raw signature'
  )
  ...
  console.log('CATALOG RESOLVER R7 HUMAN SCOPE TESTS PASSED — incomplete human answers remain line-scoped; complete exact signatures can be safely learned and fanned out.')
} catch (error) {
