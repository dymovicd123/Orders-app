...
try {
  ...
  check(!modal.includes("Само уточнение применяется только к этой позиции заказа.") && !modal.includes("matching unresolved rows"), 'Advanced resolver should not expose row-scope implementation commentary to operators')
