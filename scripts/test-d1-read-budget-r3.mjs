...
check(financeHook.includes("options: { force?: boolean; scope?: 'full' | 'finance'; reportType?: FinanceReportType }"), 'Finance scoped read option missing')
check(financeHook.includes("const key = `${scope}::${reportType || 'all'}::${range.dateFrom}::${range.dateTo}`"), 'Finance full/workspace/report caches are not isolated')
...
check(financeWorker.includes("() => financeWorkspaceOnly ? emptyRowsResult() : !needsReport('products') ? emptyRowsResult() : db.prepare(\n      `SELECT oi.product_name_snapshot AS product"), 'Finance workspace still computes product report')
