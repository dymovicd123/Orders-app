...
try {
  ...
  check(report.includes('traceScope: { startDate, endDate, selectedOperationPeriodOnly: true, includesOrderPeriodBeforePayments: true }'), 'Finance contract no longer distinguishes operation-period trace from order-period early-payment review')
...
check(`${money}\n${orderWrite}`.includes("'payment_reversal'"), 'Immutable payment-reversal evidence missing')
  ...
  check(reportUi.includes('Товары расположены по количеству проданных единиц.') && reportUi.includes('Старые заказы без цены по позиции в расчёт не входят.'), 'Product report lost honest order-period / saved-price coverage semantics')
