...
try {
  ...
  check(inventory.includes('const visibleRows = allRows.filter') && inventory.includes("if (simpleStockAvailabilityFilter === 'all') return hasExplicitStockSearch ? true : physical !== 0 || reserved !== 0"), 'search and availability filters no longer compose honestly')
  check(inventory.includes('allRows,') && inventory.includes('availabilityFilterApplied: true'), 'product truth scope is not carried into Overview')
  check(overview.includes('availabilityScopeLabel') && overview.includes('Поиск: «') && !overview.includes('фильтр наличия не скрывает найденные позиции'), 'search/filter state is not presented honestly')
