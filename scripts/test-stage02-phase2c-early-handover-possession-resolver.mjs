...
try {
  ...
  check(client.includes('await askStockResolution({') && client.includes('Подтверждение относится только к этой выдаче. Общий остаток товара этим не пересчитывается.'), 'Phase2C friendly possession confirmation missing')
