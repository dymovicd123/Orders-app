...
const overviewNavAt = nav.indexOf("{ value: 'overview' as const")
const catalogNavAt = nav.indexOf("...(isAdmin ? [{ value: 'catalog' as const, label: 'Товары'")
const movementNavAt = nav.indexOf("{ value: 'movement' as const")
const stocktakeNavAt = nav.indexOf("{ value: 'stocktake' as const")
const historyNavAt = nav.indexOf("{ value: 'history' as const")
check(catalogNavAt >= 0 && nav.includes("openInventoryPanel(entry.value)"), 'catalog must remain admin-only inside the shared Warehouse navigation')
check(overviewNavAt >= 0 && overviewNavAt < catalogNavAt && catalogNavAt < movementNavAt && movementNavAt < stocktakeNavAt && stocktakeNavAt < historyNavAt, 'admin Warehouse navigation order must stay Остатки → Товары → Операции → Проверка → История')
