...
text=p.read_text()
anchor="""      await ensureOrderItemWorkshopColumn(env.DB);

      if (url.pathname === '/api/admin-mode/status' && request.method === 'GET') {
"""
block="""      await ensureOrderItemWorkshopColumn(env.DB);
...
if text.count(anchor) != 1:
    raise SystemExit(f'router insertion anchor count={text.count(anchor)}')
