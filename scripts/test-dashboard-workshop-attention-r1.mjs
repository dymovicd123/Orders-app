...
check(!dashboardSection.includes('dashboard-scroll-list dashboard-workshop-order-list'), 'Workshop attention must not inherit the generic dashboard grid list')
check(!dashboardSection.includes('Требует реакции'), 'Workshop cards must use color and factual status instead of a redundant reaction label')
check(dashboardSection.includes('`Опоздание ${order.overdueDays} дн.`'), 'Workshop cards must show exact overdue days')
check(dashboardSection.includes('Ждёт {order.waitingDays} дн.'), 'Workshop cards must show waiting days in the visible header')
check(dashboardSection.includes('>Срочно</span>'), 'Workshop cards must show urgent status visibly')
check(dashboardSection.includes('>Есть комментарий</span>'), 'Workshop cards must show comment status visibly')
...
check(dashboardCss.includes('display: flex;\n  flex-direction: column;'), 'Workshop attention list must own a flex-column layout so rows cannot collapse')
check(dashboardCss.includes('flex: 0 0 auto;'), 'Workshop order cards must keep content-driven row height')
