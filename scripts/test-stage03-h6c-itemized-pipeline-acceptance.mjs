...
check(app.includes("import { resolveCatalogOrderSalePrice } from './app/order-pricing'"), 'Create controller no longer reaches H5 resolver')
...
check(appCreate.includes('orderTotal: createDraft.orderTotal ? Number(createDraft.orderTotal) : undefined'), 'Legacy Create order-total payload unexpectedly changed')
check(appCreate.includes('unitPrice: 0,'), 'Legacy Create line-price payload unexpectedly changed')
check(!appCreate.includes('pricingMode:'), 'Itemized Create request was activated without client-policy gate')
check(createUi.includes('Цена заказа'), 'Visible Create UI no longer preserves legacy manual total')
check(!createUi.includes('Цена позиции') && !createUi.includes('catalogPriceSnapshot'), 'Hidden pricing preparation leaked into visible Create UI')

console.log('STAGE03-H6C ITEMIZED PIPELINE ACCEPTANCE PASSED — pure itemized arithmetic, explicit server Create persistence, read/report history, edit fail-closed guard and hidden Catalog recommendation state align end-to-end while visible Create remains legacy')
