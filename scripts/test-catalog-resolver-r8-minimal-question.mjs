...
try {
  ...
  check(ux.includes("assert.ok(b.text().includes('Не указан пол'))") && ux.includes("assert.ok(!b.text().includes('В заказе написано'))") && ux.includes("assert.ok(!b.text().includes('Админ'))"), 'Actual component regression no longer proves the minimal R8 surface')
