...
const stage02Phase1BWorkshopDispositionR2Manifest = JSON.parse(fs.readFileSync(path.join(root, 'scripts/stage02-phase1b-workshop-disposition-r2-worker-manifest.json'), 'utf8'))
if (stage02Phase1BWorkshopDispositionR2Manifest?.version !== 1 || stage02Phase1BWorkshopDispositionR2Manifest?.revision !== 'stage02-phase1b-workshop-disposition-r2') throw new Error('Stage02 Phase1B Workshop disposition R2 Worker manifest invalid')
const stage02Phase1BWorkshopDispositionR2BlobSha = (value) => {
  const bytes = Buffer.from(value)
  return crypto.createHash('sha1').update(Buffer.from(`blob ${bytes.length}\0`)).update(bytes).digest('hex')
}
if (!process.env.STAGE02_PHASE1B_WORKSHOP_DISPOSITION_R2_NORMALIZED) {
  const originals = new Map()
  let childStatus = 1
  try {
    for (const [relative, delta] of Object.entries(stage02Phase1BWorkshopDispositionR2Manifest.files || {})) {
      const absolute = path.join(root, relative)
      const actual = fs.readFileSync(absolute, 'utf8')
      if (stage02Phase1BWorkshopDispositionR2BlobSha(actual) !== delta.afterGitBlob || actual.split(/\r?\n/).length !== delta.afterLines) {
        throw new Error('Stage02 Phase1B Workshop disposition R2 changed beyond exact manifest: ' + relative)
      }
      let reverted = actual
      for (const replacement of [...(delta.replacements || [])].reverse()) {
        if (!reverted.includes(replacement.afterBlock)) throw new Error('Stage02 Phase1B Workshop disposition R2 after-block missing: ' + relative)
        reverted = reverted.replace(replacement.afterBlock, replacement.beforeBlock)
      }
      if (stage02Phase1BWorkshopDispositionR2BlobSha(reverted) !== delta.beforeGitBlob || reverted.split(/\r?\n/).length !== delta.beforeLines) {
        throw new Error('Stage02 Phase1B Workshop disposition R2 predecessor reconstruction failed: ' + relative)
      }
    ...
    const child = spawnSync(process.execPath, [process.argv[1]], {
      ...
      shell: false,
      windowsHide: true,
      env: { ...process.env, STAGE02_PHASE1B_WORKSHOP_DISPOSITION_R2_NORMALIZED: '1' },
    })
  ...
  console.log('STAGE02 PHASE1B WORKSHOP DISPOSITION R2 WORKER STRUCTURAL LAYER PASSED')
  process.exit(0)
}
