...
const stage03G3ItemizedManifest = JSON.parse(fs.readFileSync(path.join(root, 'scripts/stage03-g3-itemized-calculator-worker-manifest.json'), 'utf8'))
if (stage03G3ItemizedManifest?.version !== 1 || stage03G3ItemizedManifest?.revision !== 'stage03-g3-itemized-calculator-contract') throw new Error('Stage03-G3 itemized calculator Worker manifest invalid')
const stage03G3ItemizedBlobSha = (value) => {
  const bytes = Buffer.from(value)
  return crypto.createHash('sha1').update(Buffer.from(`blob ${bytes.length}\0`)).update(bytes).digest('hex')
}
if (!process.env.STAGE03_G3_ITEMIZED_CALCULATOR_NORMALIZED) {
  const originals = new Map()
  let childStatus = 1
  try {
    for (const [relative, delta] of Object.entries(stage03G3ItemizedManifest.files || {})) {
      const absolute = path.join(root, relative)
      const actual = fs.readFileSync(absolute, 'utf8')
      if (stage03G3ItemizedBlobSha(actual) !== delta.afterGitBlob || actual.split(/\r?\n/).length !== delta.afterLines) throw new Error('Stage03-G3 itemized calculator Worker changed beyond exact manifest: ' + relative)
      let reverted = actual
      for (const replacement of [...(delta.replacements || [])].reverse()) {
        if (!reverted.includes(replacement.afterBlock)) throw new Error('Stage03-G3 itemized calculator after-block missing: ' + relative)
        reverted = reverted.replace(replacement.afterBlock, replacement.beforeBlock)
      }
      if (stage03G3ItemizedBlobSha(reverted) !== delta.beforeGitBlob || reverted.split(/\r?\n/).length !== delta.beforeLines) throw new Error('Stage03-G3 itemized calculator predecessor reconstruction failed: ' + relative)
      originals.set(relative, actual)
      fs.writeFileSync(absolute, reverted)
    }
    const child = spawnSync(process.execPath, [process.argv[1]], {cwd: root, stdio: 'inherit', shell: false, windowsHide: true, env: { ...process.env, STAGE03_G3_ITEMIZED_CALCULATOR_NORMALIZED: '1' }})
  ...
  console.log('STAGE03-G3 ITEMIZED CALCULATOR WORKER STRUCTURAL LAYER PASSED')
  process.exit(0)
}
