...
const postReviewResolverIntakeWorkerManifest = JSON.parse(fs.readFileSync(path.join(root, 'scripts/post-review-resolver-intake-worker-manifest.json'), 'utf8'))
if (postReviewResolverIntakeWorkerManifest?.version !== 1 || postReviewResolverIntakeWorkerManifest?.revision !== 'post-review-resolver-intake-ux-20260920') throw new Error('Post-review resolver/intake Worker manifest invalid')
const postReviewResolverIntakeWorkerBlobSha = (value) => {
  const bytes = Buffer.from(value)
  return crypto.createHash('sha1').update(Buffer.from(`blob ${bytes.length}\0`)).update(bytes).digest('hex')
}
if (!process.env.POST_REVIEW_RESOLVER_INTAKE_WORKER_NORMALIZED) {
  const originals = new Map()
  let childStatus = 1
  try {
    for (const [relative, delta] of Object.entries(postReviewResolverIntakeWorkerManifest.files || {})) {
      const absolute = path.join(root, relative)
      const actual = fs.readFileSync(absolute, 'utf8')
      if (postReviewResolverIntakeWorkerBlobSha(actual) !== delta.afterGitBlob || actual.split(/\r?\n/).length !== delta.afterLines) throw new Error('Post-review resolver/intake Worker changed beyond exact manifest: ' + relative)
      let reverted = actual
      for (const replacement of [...(delta.replacements || [])].reverse()) {
        if (!reverted.includes(replacement.afterBlock)) throw new Error('Post-review resolver/intake Worker after-block missing: ' + relative)
        reverted = reverted.replace(replacement.afterBlock, replacement.beforeBlock)
      }
      if (postReviewResolverIntakeWorkerBlobSha(reverted) !== delta.beforeGitBlob || reverted.split(/\r?\n/).length !== delta.beforeLines) throw new Error('Post-review resolver/intake Worker predecessor reconstruction failed: ' + relative)
      originals.set(relative, actual)
      fs.writeFileSync(absolute, reverted)
    }
    const child = spawnSync(process.execPath, [process.argv[1]], {
      cwd: root, stdio: 'inherit', shell: false, windowsHide: true,
      env: { ...process.env, POST_REVIEW_RESOLVER_INTAKE_WORKER_NORMALIZED: '1' },
    })
  ...
  console.log('POST-REVIEW RESOLVER/INTAKE WORKER STRUCTURAL LAYER PASSED')
  process.exit(0)
}
