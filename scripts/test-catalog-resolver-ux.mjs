...
assert.ok(b.text().includes('не найдено в справочнике')); assert.ok(b.text().includes('Сначала проверьте, не опечатка ли это')); assert.equal(b.count('h4'), 1)
assert.ok(b.button('Исправить / выбрать существующее')); await b.click('Добавить как новое значение'); assert.ok(b.text().includes('Не указан пол'))
assert.ok(!b.text().includes('Менеджер записал'))
...
await e.flush(); assert.equal(e.count('select'), 0); assert.equal(e.nodes().filter(n => n.type === 'button' && String(n.props.children).startsWith('ДОКЕР')).length, 4)
