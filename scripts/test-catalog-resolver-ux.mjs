...
assert.ok(modal.includes('Материал станет доступен') || modal.includes('станет доступно в следующих заказах'))
...
assert.ok(b.text().includes('Добавить?')); assert.equal(b.count('h4'), 1)
await b.click('Добавить'); assert.ok(b.text().includes('Какой здесь пол?')); await b.click('Оставить неизвестным')
...
await manager.flush(); assert.ok(!manager.text().includes('Оставить неизвестным')); assert.ok(!manager.text().includes('Расширенное исправление'))
