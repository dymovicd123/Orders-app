...
assert.ok(modal.includes('В заказе написано'))
assert.ok(modal.includes('Ничего из предложенного не подходит'))
assert.ok(modal.includes('Исправить вручную'))
assert.ok(modal.includes('Выбрать из справочника'))
assert.ok(modal.includes('Это новый'))
...
await e.click('Ничего из предложенного не подходит'); assert.ok(!e.requests.some(r => r.path === '/api/catalog')); await e.click('Открыть ручное исправление')
