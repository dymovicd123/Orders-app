...
assert.equal(rankedProducts([{ id: 6, name: 'ДАРА ШАПАН' }, { id: 7, name: 'ДВОЙКА' }], 'Д')[0].product.id, 6, 'E: one-letter prefix search should surface matching products')
assert.ok(rankedProducts([{ id: 6, name: 'ДАРА ШАПАН' }, { id: 8, name: 'АБАЙ ШАПАН' }], 'ШАПАН').every(entry => entry.score > 0), 'E: token search should keep all matching shapan candidates visible')
...
assert.ok(modal.includes('В заказе'))
assert.ok(modal.includes('Исправить вручную'))
assert.ok(modal.includes('Исправить вручную'))
assert.ok(modal.includes('Другой вариант'))
assert.ok(modal.includes('Новый товар') && modal.includes('Новый {label.toLowerCase()}'))
...
function mount(respond, admin = true) {
  ...
  const nodes = (node = tree, result = [], seen = new WeakSet()) => { if (Array.isArray(node)) node.forEach(n => nodes(n, result, seen)); else if (node && typeof node === 'object' && !seen.has(node)) { seen.add(node); result.push(node); nodes(node.props?.children, result, seen) } return result }
  const text = (node, seen = new WeakSet()) => Array.isArray(node) ? node.map(value => text(value, seen)).join('') : node && typeof node === 'object' ? (seen.has(node) ? '' : (seen.add(node), text(node.props?.children, seen))) : node == null || typeof node === 'boolean' ? '' : String(node)
...
await a.flush()
assert.ok(a.text().includes('Цвет и размер не указаны'))
assert.equal(a.count('h4'), 1)
assert.ok(!a.text().includes('Расширенное исправление'))
assert.ok(!a.requests.some(r => r.path === '/api/catalog'))
const yes = a.button('Да, не нужны'); yes.props.onClick(); yes.props.onClick(); await a.flush()
...
await c.flush(); assert.equal(c.count('h4'), 1); await c.click('Сохранить и продолжить'); assert.equal(c.completed(), 1)
const two = mount(resolveFixture([sourceItem, { ...cleanItem, orderItemId: 2 }]))
await two.flush(); await two.click('Да, не нужны'); assert.equal(two.completed(), 0); assert.ok(two.text().includes('Товар 2 из 2'))
await two.click('Сохранить и продолжить'); assert.equal(two.completed(), 1)
const bad = mount(resolveFixture([sourceItem], context, { badRecheck: true }))
await bad.flush(); await bad.click('Да, не нужны'); assert.equal(bad.completed(), 0); assert.ok(bad.text().includes('Не удалось проверить все товары'))
...
await b.flush(); assert.ok(b.text().includes('Что означает')); assert.ok(b.button('Длина')); assert.ok(b.button('Размер')); await b.click('Материал')
assert.ok(b.text().includes('Материал: ДУБАЙСКИЙ ВЕЛЮР ✓'))
assert.ok(b.text().includes('Проверьте материал')); assert.ok(b.text().includes('В заказе:')); assert.equal(b.count('h4'), 1)
assert.ok(b.button('Другой вариант')); await b.click('Новый материал'); assert.ok(b.text().includes('Новый материал')); await b.click('Добавить'); assert.ok(b.text().includes('Не указан пол'))
assert.ok(!b.text().includes('В заказе написано'))
assert.ok(!b.text().includes('Материал:'))
assert.ok(!b.text().includes('Цвет:'))
assert.ok(!b.text().includes('Размер / возраст:'))
assert.ok(!b.text().includes('Ничего из предложенного не подходит'))
assert.ok(!b.text().includes('Исправить вручную'))
...
await e.flush(); assert.equal(e.count('select'), 0); assert.equal(e.nodes().filter(n => n.type === 'button' && String(n.props.children).startsWith('ДОКЕР')).length, 6)
await e.click('Исправить вручную')
...
await workshop.flush(); assert.ok(workshop.text().includes('Подтвердите товар для Цеха')); assert.ok(!workshop.text().includes('Какой здесь пол?')); await workshop.click('Сохранить и продолжить'); assert.equal(workshop.completed(), 1)
