...
const prefixCandidates = rankedProducts([{ id: 6, name: 'ДАРА ШАПАН' }, { id: 7, name: 'ДВОЙКА' }, { id: 8, name: 'КУРТКА' }], 'Д').filter(entry => entry.score > 0)
assert.equal(prefixCandidates.length, 2, 'E: one-letter prefix must already show useful candidates')
assert.equal(prefixCandidates[0].product.name.startsWith('Д'), true)
assert.ok(rankedProducts([{ id: 9, name: 'АБАЙ ШАПАН' }, { id: 10, name: 'ДАРА ШАПАН' }], 'ШАПАН').every(entry => entry.score > 0), 'E: token search must show all matching shapan products')
...
assert.ok(modal.includes('В заказе указано'))
assert.ok(modal.includes('Не нашли правильный вариант?'))
assert.ok(modal.includes('Расширенное исправление'))
assert.ok(modal.includes('Выбрать существующее значение'))
assert.ok(modal.includes('Это новое значение'))
...
assert.ok(b.text().includes('Такого значения ещё нет среди известных')); assert.ok(b.text().includes('выберите его')); assert.equal(b.count('h4'), 1)
assert.ok(b.button('Выбрать существующее значение')); await b.click('Это новое значение'); assert.ok(b.text().includes('Не указан пол'))
assert.ok(!b.text().includes('В заказе указано'))
assert.ok(!b.text().includes('Материал:'))
assert.ok(!b.text().includes('Цвет:'))
assert.ok(!b.text().includes('Размер / возраст:'))
assert.ok(!b.text().includes('Не нашли правильный вариант?'))
...
await e.flush(); assert.equal(e.count('select'), 0); assert.equal(e.nodes().filter(n => n.type === 'button' && String(n.props.children).startsWith('ДОКЕР')).length, 5)
await e.click('Не нашли правильный вариант?'); assert.ok(!e.requests.some(r => r.path === '/api/catalog')); await e.click('Расширенное исправление')
