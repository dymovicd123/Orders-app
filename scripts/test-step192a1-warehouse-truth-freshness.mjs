...
try {
  ...
  check(!createReturnFlow.includes("isWorkshop && inventorySource === 'boutique'"), 'Workshop return still blocks explicit Boutique inventory destination')
  ...
  check(!exchanges.includes("oldItemIsWorkshop && oldReturnSource === 'boutique'"), 'Workshop exchange return still blocks explicit Boutique inventory destination')
  const receiveReturnedItemFlow = functionSection(exchanges, 'receiveReturnedItem', 'hasActiveStandaloneReturn')
  check(!receiveReturnedItemFlow.includes("isWorkshop && destination === 'boutique'"), 'Delayed Workshop receipt still blocks explicit Boutique inventory destination')
  ...
  console.log('STEP 192A1 WAREHOUSE TRUTH / FRESHNESS TESTS PASSED — Workshop return/exchange intake requires explicit Warehouse/Boutique disposition, no-stock avoids lifecycle work, and explicit intake remains freshness-gated/idempotent')
} catch (error) {
