...
try {
  check(modal.includes('>Проверить весь каталог</button>'), 'Unknown product lost the existing-catalog fallback')
  ...
  check(modal.includes('onClick={() => void openAdvanced(true)}>Это новый товар</button>'), 'Explicit direct create-new-product action disappeared')
  ...
  check(modal.includes('Уже известно из заказа'), 'Known order characteristics are not shown as context in new-product branch')
  check(modal.includes('Нет, это существующий товар'), 'New-product branch lost its escape back to existing catalog')
  check(modal.includes('>Выбрать из справочника</button>'), 'Unknown reference values no longer lead with choosing an existing dictionary value before accepting a new one')
