...
try {
  check(modal.includes('>Открыть полный каталог</button>'), 'Unknown product no longer leads with existing-catalog review')
  check(modal.includes('onClick={() => void openAdvanced()}>Открыть полный каталог</button>'), 'Existing-catalog review accidentally enters create-product mode')
