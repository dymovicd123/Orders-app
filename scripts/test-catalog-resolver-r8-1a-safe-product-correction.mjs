...
try {
  check(modal.includes('>Нужного товара нет в каталоге</button>'), 'Unknown product no longer leads with explicit existing-catalog review')
  check(modal.includes('onClick={() => void openAdvanced()}>Нужного товара нет в каталоге</button>'), 'Existing-catalog review accidentally enters create-product mode')
  ...
  check(modal.includes('>Выбрать существующее значение</button>'), 'Unknown reference values no longer lead with correction before accepting a new dictionary value')
